...
4. [Подключение к VPN](#4-подключение-к-VPN)
5. [Дополнительные транспорты](#5-дополнительные-транспорты)
   - [XHTTP](#51-xhttp)
   - [Hysteria2](#52-hysteria2)
5. [Серверный роутинг](#6-серверный-роутинг)
6. [Защита серверов](#7-защита-серверов)
...
4. Выберите язык → **1. Установка компонентов Remnawave** → **2. Установить только панель** → **1. Nginx**
5. Введите домены панели, подписки и ноды.
6. Выберите **ACME HTTP-01**, укажите email.

7. По завершении установки скрипт выведет:
...
remnawave_reverse
```
Скопируйте адрес, откройте его в браузере

8. Перейдите в **Ноды** → **Управление**
9. Вернемся к этой страничке позже, а сейчас подключаемся к серверу ноды
...
4. Выберите язык → **1. Установка компонентов Remnawave** → **4. Установить только ноду** → **1. Nginx**
5. Введите домен ноды, IP-адрес сервера панели, secret key.
6. Возвращаемся в панель → появится вкладка с нодой → кликаем на этот баннер → ищем **Secret Key (SECRET_KEY)** → Копируем его(справа кнопка есть)
7. Возвращаемся на сервер → вставляем ключ, нажимаем **Enter x2** → **y** → **Enter**
8. Выберите **ACME HTTP-01**, укажите email.
9. Ждем пока нода поднимается
...
### В панели:
1. **Пользователи** → **Зеленый Плюс +**
2. Вводим **имя для подписки(например Main/admin/friend/family)** 
3. Дату ставим на 2099 год или на другое время
4. Ставим галочку напротив **Default-Squad** (в строке **Внутренние сквады**) → **Создать**
5. Нажимаем на подписку, только что созданную, и копируем **URL подписки**

### Теперь к клиенту
1. Вставляем скопированную ссылку в строке браузера
2. Откроется окно подписки. Оно само определит вашу ОС
3. Для PC/Android/IOS скачиваем **Happ**(кнопка скачивания также встроено в подписку)
4. Открываем приложение, затем в окне подписки нажимаем на **+ Добавить подписку**, соглашаемся с переходом в программу
5. В программе: выбираем режим **TUN**, включаем и проверяем работоспособность

---

## 5. Дополнительные транспорты
